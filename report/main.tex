...
\textbf{Evaluation settings.} Both Qwen runs used the script defaults: \texttt{MAX\_FRAMES=32}, 50176 pixels per frame, greedy decoding and at most 1024 new tokens. VILA used the script default of 4 frames and float16 weights. The logs show that both models received 4 frames per video.
...
\addlinespace
\texttt{RuntimeError: Expected to mark a variable ready only once} (LoRA with reentrant gradient checkpointing under DDP) & \path{sft_7b.sh} appends extra Trainer flags & \texttt{EXTRA\_TRAIN\_ARGS} with the two flags shown in Section~\ref{sec:commands} \\
\addlinespace
...
Table~\ref{tab:types} lists accuracy by question type. The three time-related types (Temporal Grounding, Trend and Pattern, Change Detection) are the weakest types for both Qwen models, and Temporal Grounding is the weakest type for VILA. VILA scores 27.5\% on Temporal Grounding, which is close to the 25\% chance level of a four-option question. Four frames of a 6-second video are a likely reason for the Qwen results on this type, because the options of a Temporal Grounding question often differ by 0.5 seconds. The baseline outputs agree with this explanation. In \TGShortTime{} of the \TGTotal{} Temporal Grounding questions, every time that the baseline states in its reasoning is at most 1.4 seconds, and the baseline answers \TGShortTimeCorrect{} of these \TGShortTime{} questions correctly.
