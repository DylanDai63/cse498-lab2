...
\textbf{Case 3, overfitting to the answer format (QA-008909, Causation Reasoning).} The question asks for the most likely cause of a severe traffic jam. The baseline wrote that the video shows no sign of construction, accidents or a major event, pointed to the gas station beside the road, and answered D (vehicles queuing to refuel), which is correct. VILA also answered D. The fine-tuned model answered C, a construction project, with no reasoning. Construction is the cause that the baseline had ruled out from the video. Of the \Loss{} questions that fine-tuning broke, \LossPickedC{} received the answer C. The pattern suggests that the model learned to skip the reasoning step and, without reasoning, falls back on a letter prior when the question needs a comparison. The available runs do not test the explanation directly.
